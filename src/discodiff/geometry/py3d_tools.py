...
class Scale(Transform3d):
    def __init__(
        ...
        """
        Create a new Transform3d representing 3D scaling.

        Option I: Scale(xyz, dtype=torch.float32, device='cpu')
            xyz should be a tensor of shape (N, 3)

        Option II: Scale(x, y, z, dtype=torch.float32, device='cpu')
            Here x, y, and z will be broadcast against each other and
            concatenated to form the scale. Each can be:
                - A python scalar
                - A torch scalar
                - A 1D torch tensor
        """
        xyz = _handle_input(x, y, z, dtype, device, "Scale")
        super().__init__(device=xyz.device, dtype=dtype)
        N = xyz.shape[0]
        mat = torch.eye(4, dtype=dtype, device=self.device).view(1, 4, 4).repeat(N, 1, 1)
        mat[:, 0, 0] = xyz[:, 0]
        mat[:, 1, 1] = xyz[:, 1]
        mat[:, 2, 2] = xyz[:, 2]
        self._matrix = mat
...
class RotateAxisAngle(Transform3d):
    def __init__(
        ...
        """
        Create a new Transform3d representing 3D rotation about a principal axis.

        Args:
            angle: rotation angle (scalar or tensor broadcastable to (N,))
            axis: one of {"X","Y","Z"} (case-insensitive)
            degrees: True if angle is specified in degrees.
        """
        axis = str(axis).upper()
        if axis not in ("X", "Y", "Z"):
            raise ValueError(f"axis must be one of 'X','Y','Z'; got {axis!r}")

        angle_t = _handle_coord(angle, dtype=dtype, device=make_device(device or "cpu")).view(-1)
        if device is not None:
            angle_t = angle_t.to(device=make_device(device))
        if degrees:
            angle_t = angle_t * (np.pi / 180.0)
        R = _axis_angle_rotation(axis, angle_t)
        super().__init__(device=R.device, dtype=dtype)
        N = R.shape[0]
        mat = torch.eye(4, dtype=dtype, device=self.device).view(1, 4, 4).repeat(N, 1, 1)
        mat[:, :3, :3] = R.to(dtype=dtype)
        self._matrix = mat
...
def get_screen_to_ndc_transform(
    ...
    """
    Minimal screen → NDC transform used by CamerasBase helpers.

    Screen coordinates are expected in pixel units with origin at top-left.
    Output is PyTorch3D NDC with origin at image center and axes +X left, +Y up.
    """
    ...
    if image_size is None:
        raise ValueError("image_size is required for screen/NDC conversion")

    image_size = torch.as_tensor(image_size, dtype=dtype, device=device)
    if image_size.ndim == 1:
        image_size = image_size[None, :]
    if image_size.shape[-1] != 2:
        raise ValueError(f"image_size must have shape (N,2) or (2,); got {tuple(image_size.shape)}")

    H = image_size[:, 0]
    W = image_size[:, 1]

    # Map x:[0,W] → [-1,1], y:[0,H] → [-1,1] (screen NDC, +x right, +y down).
    sx = 2.0 / W
    sy = 2.0 / H
    t = Transform3d(device=device, dtype=dtype).scale(sx, sy, 1.0).translate(-1.0, -1.0, 0.0)

    # Convert screen NDC to PyTorch3D NDC (+x left, +y up) by flipping both axes.
    t = t.scale(-1.0, -1.0, 1.0)

    # API hook: if caller already handled flipping, optionally preserve parity.
    if with_xyflip:
        t = t.scale(-1.0, -1.0, 1.0)
    return t
...
def get_ndc_to_screen_transform(
    ...
    """
    Minimal NDC → screen transform used by CamerasBase helpers.

    Input is PyTorch3D NDC (+X left, +Y up). Output is pixel coordinates
    with origin top-left and axes +X right, +Y down.
    """
    device = cameras.device
    dtype = torch.float32
    ...
    if image_size is None:
        image_size = cameras.get_image_size()
    if image_size is None:
        raise ValueError("image_size is required for NDC/screen conversion")
    ...
    # PyTorch3D NDC → screen NDC (+x right, +y down)
    t = Transform3d(device=device, dtype=dtype).scale(-1.0, -1.0, 1.0)

    # screen NDC [-1,1] → pixels [0,W],[0,H]
    sx = W / 2.0
    sy = H / 2.0
    t = t.translate(1.0, 1.0, 0.0).scale(sx, sy, 1.0)

    if not with_xyflip:
        # Preserve API shape; current callers request with_xyflip=True.
        return t
    return t
