"""Remote setup helpers: git, pip, wget, model URIs (requests-backed fetch)."""
...
def download_model(
    diffusion_model_name, diff_model_map, model_path, check_model_sha, uri_index=0
):
    if diffusion_model_name != "custom":
        ...
        if os.path.exists(model_local_path) and check_model_sha:
            ...
            if file_hash == diff_model_map[diffusion_model_name]["sha"]:
                ...
                print(
                    f"{diffusion_model_name} SHA doesn't match. Will redownload it."
                )
        elif (
